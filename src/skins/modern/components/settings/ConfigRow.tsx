  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2">
            {config.help && (
              <button
                ...
                className="text-fg-dim hover:text-fg transition-colors flex-shrink-0"
              >
                {expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} className="rtl:-scale-x-100" />}
              </button>
            )}
            {!config.help && <span className="w-3" />}
            <span className="font-mono text-xs text-fg">{config.name}</span>
            {isReadonly && <Lock size={11} className="text-fg-dim flex-shrink-0" aria-label={t('Read-only')} />}
          </div>
          {config.prompt && (
            <p className="ps-5 mt-0.5 text-xs leading-snug text-fg-dim">{config.prompt}</p>
          )}
        </td>
        <td className="px-4 py-2 align-top">
          {isEditing ? (
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  aria-label={t('Save')}
                  className="p-1 rounded text-accent hover:bg-accent/15 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
            ...
            <div className="flex items-center gap-2">
              <span
                ...
                className={clsx(
                  ...
                  isReadonly
                    ? 'text-fg-dim'
                    : 'text-fg-muted cursor-pointer hover:text-fg transition-colors'
                )}
              ...
              >
                {dirtyValue !== undefined
                  ...
                  : config.value
                    ...
                    : <span className="italic text-fg-dim">{t('empty')}</span>}
              </span>
              {/* Unsaved is state: this row differs from what the server holds. */}
              {dirtyValue !== undefined && (
                <span className="text-xs px-1 rounded bg-warn/15 text-warn" title={t('Unsaved')}>{t('unsaved')}</span>
              )}
              {canReset && (
                <button
                  onClick={onReset}
                  ...
                  className="p-1 rounded text-fg-dim opacity-0 group-hover:opacity-100 focus:opacity-100 hover:text-fg hover:bg-surface-2 transition-all"
                >
      ...
      {expanded && config.help && (
        <tr>
          <td colSpan={2} className="px-4 py-2 bg-surface-2">
            <p className="text-xs text-fg-muted ps-5 whitespace-pre-line">{config.help.trim()}</p>
          </td>
        </tr>
      )}
    </>
