 ...
 * ZM operators where the Classic skin lives so they're not stranded in the
 * modern one by default. Dismissal sticks in localStorage so we don't pester
 * users.
 *
...
export function SkinHint() {
  ...
  return (
    <div
      role="status"
      className={clsx(
        'fixed bottom-4 end-4 z-40 max-w-sm',
        'rounded border border-border bg-surface shadow-[var(--elevation-2)]',
        'p-4 animate-in fade-in slide-in-from-bottom-2 duration-300',
      )}
    >
      <div className="flex items-start gap-3">
        <Sparkles size={16} className="text-fg-dim flex-shrink-0 mt-0.5" aria-hidden />
        <div className="flex-1">
          ...
          <p className="text-xs text-fg-muted leading-relaxed">
            <Trans>
              This is the modern view. If you'd rather the classic
              ZoneMinder layout, switch in{' '}
              <Link
                to="/settings"
                onClick={dismiss}
        ...
        <button
          ...
          className="flex-shrink-0 p-1 rounded text-fg-dim hover:text-fg hover:bg-surface-2 transition-colors"
        >
          <X size={14} />
        </button>
      </div>
