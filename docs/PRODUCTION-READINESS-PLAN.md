...
| F-3 ✅ | _Fixed: `Units` only scales the threshold fields; coords stay pixels, and the overlay uses the displayed (post-rotation) frame. Pinned by `src/features/zones/zoneArea.test.ts` — "treats coords as pixels even when Units is Percent" and "overlay points are the stored pixel coords regardless of Units". The corrupted dev-box zone was repaired._ **Zone units toggle corrupted geometry.** `convertUnits` rescales the polygon to 0–100 and saves it as `coords`; ZoneMinder stores coords in pixels regardless of `Units`. Monitor 1's only zone on the dev box is now a 0% sliver (motion detection effectively off). The editor also uses un-rotated width/height for ROTATE_90/270 cameras, so vertices get clamped and the overlay is wrong. `PUT /zones/{id}` accepts only `name`,`polygon`, so Type/Units changes are silently dropped while the form shows them editable. | `src/features/zones/ZoneEditor.tsx:110-149`; `src/routes/monitors/$monitorId_.zones.tsx:46-47` | Keep coords in pixel space always; use rotated dimensions; disable Type/Units on edit until BT-05 lands. **Done 2026-08-21:** all four dev-box zones (monitors 1–4 were all affected) rewritten to full-frame pixels in view dimensions via the API; `Units` stays `Percent` (ZoneMinder's default for the "All" zone; the API cannot change it). Code fix + test fix land with the skin migration. | S + data repair |
...
| F-6 ✅ | _Fixed: page, API wrapper, tests and nav entry removed — the REST API is sessionless._ **Sessions page wrote junk.** `/sessions` is ZM's PHP web-session table (`access` is a Unix timestamp, `data` is serialised PHP). The page labels `access` as a permission bitmask and "Create token" inserts fabricated rows. | `src/routes/settings/sessions.tsx`; `src/api/sessions.ts` | **Done 2026-08-21: page, API wrapper, test and nav entry removed** — the REST API is sessionless; long-lived API keys are a backend feature (BT-10). | S |
...
| F-20 ✅ | _Fixed._ ~~Rotate Logs posts to `/system/log_rotate`~~; ~~logout wrapper uses POST and is never called~~ (`GET /auth/logout`); ~~Start/Stop/Restart have no confirmation~~ — every system action now routes through `setConfirmAction` with its own title and message, and Stop/Restart are styled as destructive. | `src/api/system.ts:98-100`; `src/api/auth.ts:22-30`; `src/routes/settings/index.tsx:488-560` | S |
