...
| **3 — Parity build-out** | W9 by area in the listed order; classic rebuild of the six core pages; Test Tiers 4–7 running alongside | 75–85 d | Every FE-possible row in Section 6/W9 closed; classic fidelity ≥ 85 on the six core pages; every route has both-skin e2e | **Re-measured 2026-09-16, ~60 gaps found; ~35 closed 2026-09-17.** Six read-only audits diffed every core legacy page against both skins (`legacy-requirements/review-2026-09-16/`, local). The earlier "all built" reading was wrong: filter Monitor terms saved the name, not the id; Montage Review drew at half scale; the user editor still locked fields the backend accepts. See the 2026-09-17 done-log entry for what closed and what is still open. Classic owns all 23 pages on legacy layouts (wave 4), Tiers 4–7 are running, and the 2026-08-23 re-verification drove **every** W9 area in a browser: all are substantially built. The two real gaps it found (Watch monitor navigation, two timestamps bypassing the W7 formatter) are fixed. What is left before 1.0 is Section 10, not W9. **Still unmeasured: classic fidelity** — the ≥ 85 criterion needs fresh captures of the legacy UI, and the dev box that hosts it is down. |
...
### 2026-09-17 — parity pass over the 2026-09-16 audits

Six read-only audits (console, events + filters, event view, watch, montage/cycle/review, options/admin) diffed every core legacy page against both skins and the live API, and ranked ~60 gaps. This pass closed the frontend-possible ones an operator meets first. Unit tests went 3,532 → 3,700+ across 288 files; typecheck, lint, i18n check and the coverage floor are green.

- **Data bugs.** Filter `Monitor` terms now store the monitor **id** (ZoneMinder's `zmfilter.pl` never matched a name); `Tags` / `Group` / `AlarmedZoneId` / `StateId` / `*ServerId` / `ExistsInFileSystem` are real pickers writing what `FilterTerm.php` runs. Montage Review canvases are normalised against the mean monitor area like legacy, not drawn at half scale. User edit sends every field `UpdateUserRequest` accepts (name, phone, password, language, home view, API enabled, permissions) instead of locking them.
- **Filters.** `filter.js` validation and button gating ported (`validate.ts`): empty values and unbalanced brackets block, AutoDelete-without-Archived and UpdateDiskSpace-without-EndDateTime confirm, Save/Delete only for the owner or a System editor. Classic header: `Id Name` under `ZM_WEB_ID_ON_FILTER`, `[background]`/`[concurrent]`, read-only Id, run-as user defaulting to the operator, Email Server, `NULL Unspecified` copy/move, actions gated on `ZM_OPT_*`. Modern gained "User to run filter as". Events page size list ends at 1000 (the API's ceiling).
- **Console.** Hour/Day/Week/Month counts link to the events list already bounded by time; SELECT narrows the console to the checked rows (the `BulkModeDialog` it used to open is gone); Function cell reads `Offline` after 90 s without a status row and prints `CaptureFPS/AnalysisFPS fps bandwidth` like `console.js`; Clone opens the Add form pre-filled and saves nothing until Create; the run-state modal has New State + Save + Delete; table refresh follows `ZM_WEB_H_REFRESH_MAIN` (0 disables it).
- **Events list.** Monitor strip is a multi-select in both skins (several ids go through `/filters/preview`); Unarchive is disabled until an archived row is selected; Delete/Unarchive confirm in a dialog, Shift+click skips it, requests go in chunks of 10 with a ticker, archived rows are refused with a count; View walks the selection with autoplay; storage areas link to the list filtered by `storage_id`.
- **Event view.** Delete plays the next event or shows "No more events"; rate list −16×…¼×, Stop, ¼×…16× with legacy reverse stepping; Step Back / Step Forward by one frame while paused; replay modes None / Single / All (real gap with countdown) / Gapless; scale Auto / Actual / Fit / 480–1600 px per monitor; Prev/Next honour the list's filter, sort and page through the URL; Back is history; Download hides without a video and names the file; Zones needs System View.
- **Watch.** Events table matches `watch.js` (Notes column, `Id desc`, 20 rows, per-row trash); digital zoom on the stage with `Zoom: Nx`; iris and power PTZ wired to the endpoints that exist; PTZ shows with Control **View**; fullscreen covers the content area; Back is history.
- **Montage / Cycle / Review.** Per-tile hover controls (zoom, fullscreen, watch, edit); Show Zones on both skins; classic fullscreen; layout, stage size, speed, fit and zones persisted like the legacy cookies; `showOnHover` status position with the legacy `outsideImgBottom` default; Review gets Archived / Tags / Notes filters and the real **Fit** (`maxfit2` port, default on, hides Scale) — the old "Fit" button was legacy's **All Events** and is labelled so; cycle dwell reads `ZM_WEB_REFRESH_CYCLE`.
- **Admin.** Classic Storage table matches `_options_storage.php` (Mark column, one Delete); classic Groups matches `groups.php` (Mark, `Id Name`, monitor names, `MonitorIds[]` in the modal, saved as a diff).

**Second pass, same day (2026-09-17/18).** Event view: legacy progress bar (ten labels, score-height alarm spans, hover time), status line, hover overlay + click-to-zoom, tag shortcuts, MP4/HLS codec forcing, and the frames page's client-side sort/search/columns/CSV/All. Events list: fixed event-type Notes multi-select, Cause → edit modal, `detected:` → object-detection data, hover thumbnails, row-click select, strip toggle, pagination switch, fullscreen. Console: Name/Id link gating, Groups line, `(deleted)`, DB connections and legacy tooltips/colours in the stat strip, brand dropdown, header-flip, Log colour by `logState()`, Scan Network on `ZM_PATH_ARP`, Add on Create; the monitor filter row is multi-select with Server/Storage. Watch: cycle sidebar, filter row, viewing-timeout stop. Montage: gridstack-compatible Edit Layout (drag + resize on 48 columns, legacy save shape round-trips), ratio select, website tiles, own-layout saves; Review: 13-step speed with 0/50×, coloured timeline with score shading and collapse, monitor click/corner zoom, legacy URL params, MontageSort preference. Admin: API tokens tab, AI Datasets/Models/Classes, log event links + auto-refresh, zone polygon click/point table/self-intersection guard, control profiles trimmed to legacy's. Also: a client-side 429 gate, and the dev box's limiter reconfigured to zm-api's shipped values.

**Third pass (2026-09-18) — the dev box moved to zm-api master.** The tracked spec is what the box serves. Enums now carry ZoneMinder's DB values and the editor reads both spellings; `frame_skip` went, `device_class` and the audio-detection fields came. What master unblocked is built: zone motion settings are editable with `zone.js`'s enabling rules, Servers edits every column, Storage's DoDelete on create, per-frame JPEGs on the frames page and the alarm/snapshot stills on the event page. A read-only browser pass over the wave-2 work found four real defects (dead Back buttons, an inert log search, PTZ probes on non-PTZ cameras, the review speed label) and they are fixed. The seeded e2e suite follows the 1.39 layouts again (260 green). **Filed today:** zm-api#136–#146 for the remaining backend gaps, and **#147** because the master build selects `Monitors.DeviceClass`, which ZoneMinder 1.39.22 lacks, and in takeover mode that stopped capture on the box until the ZoneMinder package there is rebuilt from the branch head (1.39.33 schema).

**Still open from the audits, frontend-possible:** console 8 (multi-edit page), 10 (full-config export), 12 (logout modal sessions table), 13 (Zones header/footer links — no all-zones page exists); watch 12 (V4L settings modal, no Local monitors to test); events-and-filters 15 (timeline page); event-view F3 (frame viewer / stats routes), S1 (stats page); montage 11 (`fid` on the event link — `parseEventNavSearch` has no frame param); options 3 (the single-form Options page), 4 (monitor editor: presets/ONVIF buttons, classic tab order). **Backend-blocked:** watch 1, 2, 4, 5, 7, 8, 13; events 4, 5, 6, 7, 8, 9, 11, 18, 19; event-view 8 (`path`), 11, 15, F2, F4; montage 5, 12, 15 (`Deleted` monitors); options 2, 5, 6b, 6c, 7 (level multi-select, components list, id delete), 8, 14 — each already has, or needs, a zm-api ticket (Section 7).

**Noted while checking in a browser:** the dev box's zm-api rate limiter answers a normal page load with 429s (every page fires 8–12 requests, and the client's retry multiplies them). The classic Watch page and Montage Review Fit were verified in the browser only as far as the throttle allowed; the rest of this pass is verified by unit tests against MSW.
