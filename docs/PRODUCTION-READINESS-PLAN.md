...
| F-25 | **Known, unfixed.** Montage Review: picking a custom range that does not overlap the current one leaves the playhead clamped to the *old* range's edge instead of parking it at the new range's start. Reproduced and pinned by a skipped test (`it.skip('BUG: a non-overlapping custom range should park the playhead at its start')`), so the fix has its assertion waiting. | `src/features/montagereview/useMontageReviewPage.extra.test.ts:170`; `useMontageReviewPage.ts` clock clamp | S |
